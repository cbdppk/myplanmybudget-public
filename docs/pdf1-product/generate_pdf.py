...
def main() -> None:
    ...
    source = root / "docs" / "pdf1-product" / "PDF1_PRODUCT.md"
    out_primary = root / "docs" / "MyplanMybudget_PDF1_Product_UX.pdf"
    out_legacy = root / "docs" / "MyplanMybudget_PDF1_Product_UX (1).pdf"

    items = parse_markdown(source.read_text(encoding="utf-8"))
    streams = render_pages(items)
    pdf = build_pdf(streams)

    out_primary.write_bytes(pdf)
    out_legacy.write_bytes(pdf)
